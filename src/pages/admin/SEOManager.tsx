...
export default function SEOManager() {
  ...
  const forceRefreshSEO = () => {
    console.log('🔄 Force refreshing SEO cache and notifying all components...');
    
    // Clear SEO cache
    import('@/services/seoApi').then(({ clearSEOCache }) => {
      clearSEOCache();
    ...
    // Dispatch update event to all SEO components
    const seoUpdateEvent = new CustomEvent('seo-updated', {
      detail: { pageType: 'all' }
    });
    window.dispatchEvent(seoUpdateEvent);
    
    console.log('📢 SEO update event dispatched for all pages');
    
    toast({
      title: 'SEO Cache Cleared',
      description: 'All SEO components have been notified to refresh',
    });
  };
  ...
  const forceClearAllCaches = () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'All caches have been cleared. Please refresh the page.',
      });
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Clear SEO cache to force fresh data
      import('@/services/seoApi').then(({ clearSEOCache }) => {
        clearSEOCache();
        console.log('✅ SEO cache cleared after save');
      }).catch(error => {
        console.error('❌ Error clearing SEO cache:', error);
      });
      
      // Dispatch custom event to notify all SEO components to refresh
      const seoUpdateEvent = new CustomEvent('seo-updated', {
        detail: { pageType: activeTab }
      });
      window.dispatchEvent(seoUpdateEvent);
      
      console.log(`📢 SEO update event dispatched for ${activeTab}`);
